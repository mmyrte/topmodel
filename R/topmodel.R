...
#'
#' @param parameters A vector or a matrix containing the input parameters (see Details for the exact structure)
#' @param topidx A 2 column matrix with respectively the topographic index classes and values (see Details for the exact structure)
#' @param delay Delay function for overland flow (see Details)
#' @param rain A vector of rain data (m per timestep)
#' @param ETp A vector of potential evapotranspiration data (m per timestep)
#' @param verbose If set to TRUE, returns in addition to predicted discharge also overland flow, base flow and saturated zone storage
#' @param Qobs If Qobs is given, normal output is suppressed and only the Nash and Sutcliffe efficiency is returned (m per timestep)
#'
#' @details
...
#'
#' @return The function returns an array of observed discharges. If more than one parameter set is given, a matrix is returned, with each column representing a discharge set coinciding with the parameter sets. If Qobs is given, the function returns an array of Nash-Sutcliffe efficiencies, 1 for each parameter sets.
#'
...
topmodel <- function(
  ...
  if (length(parameters) / iterations < 11) {
    stop("Incorrect number of parameters")
  }
  if (length(rain[is.na(rain)]) != 0) {
    stop("Rain contains incorrect values such as NA's")
  }
  ...
  if ((length(Qobs) == 1) & all(is.na(Qobs))) {
    ...
    if (
      length(Qobs[Qobs >= 0]) != length(Qobs) || length(Qobs) != length(rain)
    ) {
  ...
  if (all(Qobs == -9999) & (iterations > 1) & (v == 1)) {
    result <- matrix(result, ncol = iterations)
  }
